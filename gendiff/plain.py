...
def format_changes(data, parent_key=''):
    ...
    for item in data:
        ...
        current_key = f"{parent_key}.{item['key']}" if parent_key else item['key']

        # Обрабатываем изменение статуса, исключая верхние уровни
        if item['status'] == 'added':
            value = item['old_value'] if item['old_value'] != 'null' else None
            result.append(f"Property '{current_key}' was added with value: {format_value(value)}")
            ...
            if old_value != new_value:  # Проверяем, что значения отличаются
                if parent_key:  # Только если это не верхний уровень
                    result.append(f"Property '{current_key}' was updated. From {format_value(old_value)} to {format_value(new_value)}")
