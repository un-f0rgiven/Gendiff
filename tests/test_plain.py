...
def test_format_changes():
    ...
    assert format_changes(data_added) == ["Property 'property1' was added with value: null"]

    data_removed = [{'key': 'property1', 'status': 'removed'}]
    assert format_changes(data_removed) == ["Property 'property1' was removed"]

    data_updated = [{'key': 'property1', 'status': 'updated', 'old_value': 'old_value', 'new_value': 'new_value'}]
    assert format_changes(data_updated) == ["Property 'property1' was updated. From 'old_value' to 'new_value'"]
    ...
    nested_data = [{
        ...
        'old_value': None,
        'children': [
            {'key': 'sub_property1', 'status': 'updated', 'old_value': 'old', 'new_value': 'new'}
        ]
    }]
...
def test_print_changes():
    data = [
        ...
        {'key': 'property3', 'status': 'updated', 'old_value': 'old_value', 'new_value': 'new_value'}
    ]
    expected_output = (
        "Property 'property1' was added with value: null\n"
